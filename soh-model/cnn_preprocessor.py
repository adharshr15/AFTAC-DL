###
# Generate .npy sequences based on the .csv data given
# ###
...
data_path = "./data/"
DATA_FILES = ["synthetic_data_independent_failures_5.csv", "synthetic_data_independent_failures_4.csv"]
...
def main():
    test_split = float(input("Enter test split fraction (default is 0.2): "))
    val_split = float(input("Enter validation split fraction (default is 0.25): "))
    ...
    scaler_params_path = os.path.join(data_path, "finalized", "scaler_params.txt")
    ...
    os.makedirs(os.path.join(data_path, "finalized"), exist_ok=True)
    
    # Save processed data
    np.save(os.path.join(data_path, "finalized", f"X_train_feature_{SELECT_MODEL}.npy"), X_train_feature)
    np.save(os.path.join(data_path, "finalized", f"Y_train_feature_{SELECT_MODEL}.npy"), Y_train_feature)
    np.save(os.path.join(data_path, "finalized", f"X_val_feature_{SELECT_MODEL}.npy"), X_val_feature)
    np.save(os.path.join(data_path, "finalized", f"Y_val_feature_{SELECT_MODEL}.npy"), Y_val_feature)
    np.save(os.path.join(data_path, "finalized", f"X_test_feature_{SELECT_MODEL}.npy"), X_test_feature)
    np.save(os.path.join(data_path, "finalized", f"Y_test_feature_{SELECT_MODEL}.npy"), Y_test_feature)
