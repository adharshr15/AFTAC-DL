...
Synthetic temperature-sensor data generator with:
  ...
  • Spikes followed by a noisy exponential decay period ,
  ...
  • command-line interface (works in Spyder, terminal).
...
#################################################################################################################################################
# ------------------------------------------------------------
# Helper Method: parse strings like "12h", "30m", "10s"
# ------------------------------------------------------------
...
###############################################################################################################################################
# ------------------------------------------------------------
# Core data generator – one sensor
# ------------------------------------------------------------
def generate_sensor_series(
    ...
    daily_mean: float = 25.0,        # average temperature over 24h
    ...
    # ── Decay parameters (replaces cooldown)
    decay_len_min: int = 300,        # minimum decay length in seconds
    decay_len_max: int = 900,        # maximum decay length in seconds
    decay_noise_multiplier: float = 5.0, # factor to multiply normal noise during decay
    decay_rate: float = 0.05,        # decay constant (smaller = slower decay)
    ...
    label == 1 → anomaly (high-temperature spike or decay period).
    """
    if rng is None:
    ...
    state = "normal"
    spike_end = None
    decay_start_temp = None
    decay_end = None
    decay_length_seconds = None

    cur = start

    # Pre-calculate step in seconds for decay time tracking
    step_seconds = step.delta.total_seconds()
    time_elapsed_in_decay = 0.0
    ...
    while cur <= end:
        ...
        if state == "normal":
            ...
            if cur >= spike_end:
                # ---- spike finished → start decay -----------------------
                state = "decay"
                decay_length_seconds = rng.integers(decay_len_min, decay_len_max + 1)
                decay_end = cur + pd.Timedelta(seconds=decay_length_seconds)
                decay_start_temp = spike_temp # Start decay from the spike temperature
                time_elapsed_in_decay = 0.0

                # Temperature for the first step of decay (still looks like a spike)
                temperature = decay_start_temp + rng.normal(0, noise_std * decay_noise_multiplier)
                label = 1
            else:
                # ---- still in spike ---------------------------------
                temperature = spike_temp + rng.normal(0, noise_std)
            ...
            if cur >= decay_end:
                ...
                # Exponential decay towards the normal temperature
                temp_range = decay_start_temp - normal_temp
                decay_factor = np.exp(-decay_rate * time_elapsed_in_decay)

                decaying_temp = normal_temp + temp_range * decay_factor

                # Add high noise and keep label 1
                temperature = decaying_temp + rng.normal(0, noise_std * decay_noise_multiplier)
                label = 1

                # Update elapsed time for the next step
                time_elapsed_in_decay += step_seconds
...
# ------------------------------------------------------------
# Multi-sensor ( per-sensor randomisation)
# ------------------------------------------------------------
...
def plot_sensor_data(df: pd.DataFrame, sensor_ids: list | None = None, figsize=(14, 5)):
    ...
    plt.title("Synthetic temperature streams – normal vs. anomaly (with noisy decay)")
    plt.legend()
    plt.tight_layout()
    plt.show()
#################################################################################################################################################
...
def write_csvs(df: pd.DataFrame, folder: Path = Path("simulated_dataset")):
    ...
    print(f"Saved: {normal_path}    (rows: {len(df[df['label']==0])})")
    print(f"Saved: {anomaly_path} (rows: {len(df[df['label']==1])})")

##################################################################################################################################################
# ------------------------------------------------------------
# Demo (runs when the file is executed)
# ------------------------------------------------------------
def _demo():
    """Run a quick demo – generate, save, and plot."""
    print("\n=== Temperature-sensor simulation demo (randomised anomalies with noisy decay) ===\n")
    df = generate_multi_sensor_dataset(
        ...
        decay_len_min=300,
        decay_len_max=600,
        decay_noise_multiplier=8.0, # High noise during decay
        decay_rate=0.015, # Slow decay
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "spikes and optional day/night cycle. Includes a noisy exponential decay phase."
        )
    )
    ...
    # NEW decay parameters for CLI
    parser.add_argument("--decay-len-min", type=int, default=300,
                         help="Minimum decay length (seconds) after a spike")
    parser.add_argument("--decay-len-max", type=int, default=900,
                         help="Maximum decay length (seconds) after a spike")
    parser.add_argument("--decay-noise-mult", type=float, default=5.0,
                         help="Noise multiplier during the decay phase")
    parser.add_argument("--decay-rate", type=float, default=0.05,
                         help="Exponential decay rate (lambda). Smaller is slower.")
    ...
    df_demo = generate_multi_sensor_dataset(
        ...
        # Pass NEW decay parameters
        decay_len_min=args.decay_len_min,
        decay_len_max=args.decay_len_max,
        decay_noise_multiplier=args.decay_noise_mult,
