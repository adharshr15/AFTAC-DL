...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	...
	AI_ALIGNED(4) ai_u8 activations[AI_SOH_MODEL_DATA_ACTIVATIONS_SIZE];

	AI_ALIGNED(4) ai_u8 in_data[AI_SOH_MODEL_IN_1_SIZE_BYTES];
	AI_ALIGNED(4) ai_u8 out_data[AI_SOH_MODEL_OUT_1_SIZE_BYTES];
...
//  MX_FATFS_Init();
  ...
  HAL_TIM_Base_Start(&htim14);
	...
	if (ai_input) ai_input[0].data = AI_HANDLE_PTR(in_data);
	if (ai_output) ai_output[0].data = AI_HANDLE_PTR(out_data);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    // Fill input data
    ai_float* input_array = (ai_float *)in_data;
    for (uint32_t i = 0; i < AI_SOH_MODEL_IN_1_SIZE; i++)
    {
      input_array[i] = (ai_float)(i);
    }

    /* Print first few inputs for debugging */
    buf_len = sprintf(buf, "Inputs (first 5): ");
    HAL_UART_Transmit(&huart1, (uint8_t *)buf, buf_len, 100);
    
    for (int i = 0; i < 5 && i < AI_SOH_MODEL_IN_1_SIZE; ++i) {
      // Manual float conversion (sprintf %f may not be enabled)
      int int_part = (int)input_array[i];
      int frac_part = (int)((input_array[i] - int_part) * 100);
      if (frac_part < 0) frac_part = -frac_part;
      buf_len = sprintf(buf, "%d.%02d ", int_part, frac_part);
      HAL_UART_Transmit(&huart1, (uint8_t *)buf, buf_len, 100);
    }
    
    buf_len = sprintf(buf, "... (total %d)\r\n", AI_SOH_MODEL_IN_1_SIZE);
    HAL_UART_Transmit(&huart1, (uint8_t *)buf, buf_len, 100);
    ...
    int y_int = (int)y_val;
    int y_frac = (int)((y_val - y_int) * 100);
    if (y_frac < 0) y_frac = -y_frac;
    buf_len = sprintf(buf,
          "Output: %d.%02d | Duration: %lu\r\n",
          y_int, y_frac,
