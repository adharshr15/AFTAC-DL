...
struct OutputStageEvalBufferImpl<OutputStageScaleInt32ByFixedPointAndExponent,
  ...
  OutputStageEvalBufferImpl(const OutputStage& s) : output_stage(s) {
    left_shift = std::max(static_cast<int32_t>(0), output_stage.result_exponent);
    right_shift = std::max(static_cast<int32_t>(0), static_cast<int32_t>(-output_stage.result_exponent));
  }
